...
function DoorService.new(session)
	...
	self.doorHpBarTemplate = ReplicatedStorage:WaitForChild("Assets"):WaitForChild("UI"):WaitForChild("DoorHpBar")
...
function DoorService:_getTutorialStep(player)
	if not player then
		return nil
	end
...
function DoorService:_checkTutorialDoorUpgradeAllowed(player)
...
function DoorService:_getRoomSockets(room)
	...
	-- 命名 Room_1_BossTarget / Room_2_BossTarget ...
...
function DoorService:_getDoorHpBarWorldCFrame(door)
	if door and door.root and door.root.Parent then
		return door.root.CFrame
	end
	if door and door.doorSocket and door.doorSocket.Parent then
		return door.doorSocket.CFrame
	end
	return nil
end

function DoorService:_spawnDoorHpBar(door)
	if not door or not door.runtimeFolder or not self.doorHpBarTemplate then
		return nil
	end

	local oldHpBar = door.hpBarPart
	if oldHpBar then
		oldHpBar:Destroy()
		door.hpBarPart = nil
	end

	local hpBar = self.doorHpBarTemplate:Clone()
	hpBar.Name = "DoorHpBar"
	hpBar.Parent = door.runtimeFolder

	local worldCFrame = self:_getDoorHpBarWorldCFrame(door)
	if worldCFrame then
		setInstanceWorldCFrame(hpBar, worldCFrame)
	end

	for _, obj in ipairs(hpBar:GetDescendants()) do
		if obj:IsA("BasePart") then
			obj.Anchored = true
			obj.CanCollide = false
			obj.CanTouch = false
	...
	door.hpBarPart = hpBar
	return hpBar
end

function DoorService:_destroyDoorHpBar(door)
	if not door then
		return
	end
	if door.hpBarPart then
		door.hpBarPart:Destroy()
		door.hpBarPart = nil
	end
end
...
function DoorService:_replaceDoorModel(door, newLevel)
	...
	local hpBarWorldCFrame = self:_getDoorHpBarWorldCFrame(door)
	if door.hpBarPart and hpBarWorldCFrame then
		setInstanceWorldCFrame(door.hpBarPart, hpBarWorldCFrame)
	end
...
function DoorService:TryUpgradeDoor(player)
	if not self:_checkTutorialDoorUpgradeAllowed(player) then
		return false
	end
	...
	StatsModule.add(player, StatsModule.KEY.DoorUpgradeCount, 1, "UpgradeDoor")
	...
	if player:GetAttribute("BattleTutorialSession") == true then
		AnalyticsModule.logTutorialDoorUpgraded(player, battleFunnelSessionId)
	end
...
function DoorService:TryRepairDoor(player)
	local step = self:_getTutorialStep(player)
	if step ~= nil and step ~= "Battle_Complete" then
		return false
	end
